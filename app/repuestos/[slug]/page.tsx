...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const product = await prisma.product.findFirst({ where: { slug, activo: true, tipo: "repuesto" }, select: await productSelectWithOptionalBackground() });
  if (!product) return { title: "Repuesto no encontrado | Tecnova Perú" };
  const image = productImage(product);
  const keywords = parseJsonArray<string>(product.tags).join(", ");
  return {
    title: product.seoTitulo || `${product.nombre} | Repuestos Tecnova Perú`,
    description: product.seoDesc || product.descripcionCorta,
    keywords: keywords || `${product.nombre}, ${product.category.nombre}, ${product.marca || "Tecnova"}, repuestos industriales Perú`,
    alternates: { canonical: `/repuestos/${product.slug}` },
    openGraph: {
...
export default async function SparePartPage({ params }: Props) {
  const { slug } = await params;
  ...
  return (
    <main className="min-h-screen bg-white text-tecnova-dark">
      ...
      <SparePartDetail product={product} related={related} whatsapp={settings.whatsapp} />
      <SiteFooter settings={settings} />
    </main>
  );
}

function SparePartDetail({ product, related, whatsapp }: { product: ProductWithCategory; related: ProductWithCategory[]; whatsapp: string }) {
    ...
    <section className="mx-auto max-w-[1540px] px-4 py-8 sm:px-5 lg:px-14">
      ...
      {related.length > 0 && (
        <section className="mt-10">
          <h2 className="text-3xl font-black tracking-[-0.05em] sm:text-5xl">Repuestos relacionados</h2>
...
function buildProductJsonLd(product: ProductWithCategory, image: string) {
  ...
  return {
    ...
    offers: {
      ...
      url: `${baseUrl}/repuestos/${product.slug}`,
    },
  };
}
